...
func TestMigrateOfficialRemarkTemplateDefault(t *testing.T) {
	dbDir := t.TempDir()
	...
	const officialDefault = "{{INBOUND}}|📊{{TRAFFIC_LEFT}}|⏳{{DAYS_LEFT}}D"
	if err := db.Create(&model.Setting{Key: "remarkTemplate", Value: officialDefault}).Error; err != nil {
	...
	if got.Value != "{{INBOUND}}-{{EMAIL}}" {
		t.Fatalf("remarkTemplate = %q, want {{INBOUND}}-{{EMAIL}}", got.Value)
	}
}
