...
require_repo_files() {
    ...
    [[ "${ref}" == "main" ]] && return 0
    for name in "$@"; do
        status=$(${curl_bin} -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/Fourgetu/3x-ui/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
update_x-ui() {
    ...
    if [[ -n "${XUI_UPDATE_TAG}" ]]; then
        ...
        tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/Fourgetu/3x-ui/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/Fourgetu/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2> /dev/null
    if [[ $? -ne 0 ]]; then
    ...
    rm -f "${archive}.sha256"
    sidecar_code=$(${curl_bin} -sL --retry 3 --retry-delay 3 --connect-timeout 15 --max-time 60 -o "${archive}.sha256" -w '%{http_code}' "https://github.com/Fourgetu/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz.sha256" 2> /dev/null)
    ...
    ${curl_bin} -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/Fourgetu/3x-ui/${script_ref}/x-ui.sh" > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        ${curl_bin} -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/Fourgetu/3x-ui/${script_ref}/x-ui.rc" > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        service_unit_url="https://raw.githubusercontent.com/Fourgetu/3x-ui/${script_ref}/x-ui.service.debian"
                        ;;
                    arch | manjaro | parch)
                        service_unit_url="https://raw.githubusercontent.com/Fourgetu/3x-ui/${script_ref}/x-ui.service.arch"
                        ;;
                    *)
                        service_unit_url="https://raw.githubusercontent.com/Fourgetu/3x-ui/${script_ref}/x-ui.service.rhel"
                        ;;
                esac
